...
void setMap(Drone *d,Map *m){
	...
	if((d->currLocation.point[0] == m->base[0].point[0]) && (d->currLocation.point[1] == m->base[0].point[1])){
		...
		if(m->map[d->currLocation.point[0]][d->currLocation.point[1]].item == 'x'){
			...
			m->map[d->currLocation.point[0]][d->currLocation.point[1]].item = '^';
		}
	}
}
...
void removeOld(Map *m,Coord c){
		...
		//	Sets print out item
		if (m->map[c.point[0]][c.point[1]].item == 'x'){
	...
	if(m->map[locked.point[0]][locked.point[1]].locked == 1){
			switch(lock){
				case 1:		//Up is blocked
					if(c.point[1] == 25){			//Check if right side is edge of map
						if(c.point[0] == 25){		//Checks if corner
							setAvoid(m,d,c,0,1);
							d->avoid2 = d->avoid;
							d->avoid = 3;
						}
					...
					else if(c.point[1] == -25){		//Check if left side is edge of map
						if(c.point[0] == 25){		//checks if corner
							setAvoid(m,d,c,1,1);
							d->avoid2 = d->avoid;
							d->avoid = 4;
					...
					else if(c.point[0] == 25){		//Checks if bottom of map
...
int move(Drone *d,Map *m,int state){
	...
	//If drone is at a runway move and free it
	if(m->map[d->currLocation.point[0]][d->currLocation.point[1]].item == 'x'){
		for(i = 0;i < 4;i++){
